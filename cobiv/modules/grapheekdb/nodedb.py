...
class NodeDb(Entity):
    ...
    def search_tag(self, *args):
        ...
        if len(to_include) == 0:
            ...
            for arg in to_include:
                ...
                if first:
                    resultset = self.g.V(kind=NODE_TYPE_TAG, type=kind, value__startswith=value).inE(kind="has").inV(
                        kind=NODE_TYPE_FILE)
                    first = False
                else:
                    resultset = resultset.aka("x").outE(kind="has").outV(kind=NODE_TYPE_TAG, type=kind,
                                                                         value__startswith=value).collect("x")
        ...
        for arg in to_exclude:
            ...
            for n in self.g.V(kind=NODE_TYPE_TAG, type=kind, value__startswith=value).inE(kind="has").inV(
                    kind=NODE_TYPE_FILE):
    ...
    def add_tag(self, *args):
        for filename in self.current_imageset.current:
    ...
    def list_tags(self):
        for filename in self.current_imageset.current:
            for n in self.g.V(name=filename, kind=NODE_TYPE_FILE).outE(kind="has").outV(kind=NODE_TYPE_TAG,
                                                                                        type=TAG_TYPE_TAG):
                print n.value
